...
<br>
`use chatDB` : chatDB 라는 선택 데이터 베이스 사용  
...
####  Collection  ?
  ...
  RDBS 의 **테이블**과 비슷한 개념이지만 스키마를 따로 가지고 있지는 않는다
...
프로토콜|WebSocket|SSE
배터리소모량|큼|적음
사용처|(리얼 타임) 채팅, 주식거래|(알람 기능) 트위터,페이스북 등


Socket 과 SSE 의 가장 큰 차이는 Socket 은 양방향 통신인 점이고, SSE 는 일방향(데이터 수신)만 가능하다는 점이다.
